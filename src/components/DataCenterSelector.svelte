<script>
	...
	let label = "Data Center"
	export let data_centers;
	export let on_select_dc

	function selectDC(data_center){
		label = data_center.name
		dropdownOpened = false
		on_select_dc(data_center)
	}
</script>
		...
		>
			...
			>
				{#each data_centers as data_center}
					<li><div on:click={()=>selectDC(data_center)} on:keydown={()=>{}} class="mx-2 h-10 rounded-lg pl-3 cursor-pointer flex items-center hover:bg-blue-700">{data_center.name}</div></li>
				{/each}
			</ul>
		</div>
	</div>
